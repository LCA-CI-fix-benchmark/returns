...
class _PartialFunctionReducer(object):
    ...
    1.  Then, we run typechecking on this intermediate function
        and passed arguments to ensure correctness.
    2.  Next, we subtract intermediate arguments from the function being passed.
    3.  Finally, we perform type substitution on the newly created final function
        to replace generic variables that are already known for accuracy.
